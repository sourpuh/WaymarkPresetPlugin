    ...
    public static class Win32Clipboard
    {
        public static void CopyTextToClipboard(string str)
        {
            ...
            if (EmptyClipboard())
            {
                ...
                var bytes = GetNullTerminatedByteArrayFromString(str);

                //	Allocate the moveable memory required by the clipboard, and copy the data to the clipboard if everything went ok.
                var copiedToClipboardSuccessfully = false;
                var hNewMem = GlobalAlloc(GMEM_MOVEABLE | GMEM_ZEROINIT, new UIntPtr((uint)bytes.Length));
                if (hNewMem != IntPtr.Zero)
                {
                    var pLockedMem = GlobalLock(hNewMem);
                    if (pLockedMem != IntPtr.Zero)
                    {
                        ...
                        try
                        {
                            ...
                            copiedToClipboardSuccessfully =
                                SetClipboardData(CF_UNICODETEXT, hNewMem) != IntPtr.Zero;
                        }
                        catch
                        {
                        }
        ...
        private static byte[] GetNullTerminatedByteArrayFromString(string str)
        {
            ...
            var numBytes = sizeof(char) * (uint)(chars.Length + 1);
            ...
            for (var i = bytes.Length - sizeof(char); i < bytes.Length; ++i)
            {
                bytes[i] = 0;
            }
